...
public:
    ...
    elastic_point(float start_x, float stiffness = 0.1f, float damping = 0.5f, float mass = 1.0f, float targ_x = 0.f)
        : position_x(start_x), target_x((targ_x != 0.f) ? targ_x : start_x), velocity_x(0), stiffness(stiffness), damping(damping), mass(mass) {}
